...
**Direction is one-way.** `greybeard-data/output/on-call/runbooks/{repo}/` stays the source of truth; this phase copies *from* there *into* the app repo. Never sync back the other way — edits happen in the canonical store, then flow out here.
...
- The app-repo clone at `../greybeard-data/sources/{repo}/` has a pushable `origin` and `gh` is authenticated (`gh auth status`). This clone is what we push from — **not** the user's live working checkout.
...
```bash
git -C ../greybeard-data/sources/{repo} fetch origin
git -C ../greybeard-data/sources/{repo} checkout main
git -C ../greybeard-data/sources/{repo} pull --ff-only
git -C ../greybeard-data/sources/{repo} status --short   # must be clean
```
...
- **Sync each domain subtree with `--delete`** so renamed/removed scenarios propagate — this is safe because it only touches that domain subdir:
  ```bash
  SRC=../greybeard-data/output/on-call/runbooks/{repo}
  DST=../greybeard-data/sources/{repo}/docs/runbooks
  for d in "$SRC"/*/; do
    domain=$(basename "$d")
...
```bash
git -C ../greybeard-data/sources/{repo} add docs/runbooks
git -C ../greybeard-data/sources/{repo} status --short
git -C ../greybeard-data/sources/{repo} diff --staged --stat
```
