...
| `implement/` | Ticket/requirements → smallest viable design → test-first implementation → `review --fix` → correction → render check for UI changes → draft PR only with `--pr` |
