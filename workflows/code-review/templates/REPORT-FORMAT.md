...
The canonical output format for the code-review workflow. Both `${CLAUDE_PLUGIN_ROOT}/workflows/code-review/AGENTS.md` and the `/review` skill point here — edit this file, not the copies.
...
**Order by severity, not by lens.** Most severe first. Severity means how much damage the bug does, how hard it is to undo, and **how many people reach it**. Name the population and the path that gets them there before you rank: "every member renewed at plan-year lock" outranks "a member whose address fails validation," even when the second bug is the more interesting one. A change that skips the default path for the largest group is a failure, however small the code is; a clever edge case that needs three conditions to line up is usually a nit. Silent data corruption outranks a missing test, which outranks an unmemoized calculation.
