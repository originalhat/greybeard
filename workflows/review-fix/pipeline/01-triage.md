...
- **`ask-user`, always: a pending console check.** When a finding's falsifier line reads `pending console check`, whether it is real depends on production data that nobody has read yet. Route it to the human with the `## Data check` snippet attached, and never auto-apply it on a hunch.
